SIP.CertificationEngine=(function(){
  function assess(master,diagnostics,persistence){
    ...
    var certified=checks.noFatalErrors&&checks.hasRecords;
    var isFullyCertified=certified&&errors.length===0&&checks.calendar;
    var status=isFullyCertified?'CERTIFIED':(certified?'PROVISIONAL':'NOT_CERTIFIED');
