SIP.BusinessCalendar = (function () {
  ...
  function build(parsed,context,settings) {
    ...
    if(declared&&declared!==calendarTotal)context.diagnostics.issue('WARN','CALENDAR_SOURCE_TOTAL_MISMATCH','Calendar-derived working days differ from source working days',{calendarTotal:calendarTotal,sourceTotal:declared,authoritativeSource:'Raw Data',period:currentPeriod.periodStart});
  ...
  function shiftDate(iso,days){if(!iso||typeof iso!=='string'||!/^\d{4}-\d{2}-\d{2}/.test(iso))return'';var d=new Date(iso.slice(0,10)+'T00:00:00Z');if(isNaN(d.getTime()))return'';d.setUTCDate(d.getUTCDate()+Number(days||0));return d.toISOString().slice(0,10);}
