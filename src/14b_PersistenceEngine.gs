SIP.PersistenceEngine=(function(){
  ...
  function persist(spreadsheet,master,config){
    ...
    var masterResult={rows:0,persisted:false,archived:true,runtimeRecords:master.records.length,reason:'Certified runtime/cache model replaces the redundant physical Master Dataset'};
    ...
    return {verified:masterResult.runtimeRecords===master.records.length&&calendarResult.rows===master.calendar.rows.length,master:masterResult,calendar:calendarResult,hierarchy:hierarchyResult,relationships:relationshipResult,durationMs:Date.now()-started};
  }
