...
| Momentum | Current daily Sales | daily `SALES_AMOUNT` | latest up-to-3 matured working days vs preceding equal working-day window | `momentumPct`, `momentumDirection`, `forecastBase.momentumBase` | Momentum card/forecast/risk | Percentage and direction share one result; cutoff and working-day scope enforced |
